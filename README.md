<!-- Header animated banner -->
<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=200&section=header&text=Zafar%20Kodirov&fontSize=60&fontColor=fff&animation=twinkling&fontAlignY=35&desc=Flutter%20Developer%20%7C%20UI%2FUX%20Designer&descAlignY=55&descSize=18" width="100%"/>
</div>
<!-- Typing animation -->
<div align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&pause=1000&color=00E5FF&center=true&vCenter=true&width=600&lines=Flutter+Developer+%F0%9F%90%A6;UI%2FUX+Designer+%F0%9F%8E%A8;Mobile+App+Builder+%F0%9F%93%B1;Open+for+Freelance+%F0%9F%92%BC;16+y%2Fo+from+Uzbekistan+%F0%9F%87%BA%F0%9F%87%BF" alt="Typing SVG" />
</div>
<br/>
<!-- Badges row -->
<div align="center">
  <img src="https://img.shields.io/badge/Flutter-02569B?style=for-the-badge&logo=flutter&logoColor=white"/>
  <img src="https://img.shields.io/badge/Dart-0175C2?style=for-the-badge&logo=dart&logoColor=white"/>
  <img src="https://img.shields.io/badge/Figma-F24E1E?style=for-the-badge&logo=figma&logoColor=white"/>
  <img src="https://img.shields.io/badge/Firebase-FFCA28?style=for-the-badge&logo=firebase&logoColor=black"/>
  <img src="https://img.shields.io/badge/Adobe%20XD-FF61F6?style=for-the-badge&logo=adobe-xd&logoColor=white"/>
</div>
<br/>

🚀 About Me
dartclass ZafarKodirov {
  final String name     = "Zafar Kodirov";
  final int    age      = 16;
  final String location = "Uzbekistan 🇺🇿";
  final String role     = "Flutter Developer & UI/UX Designer";

  final List<String> skills = [
    "Pixel-perfect UI",
    "Mobile App Development",
    "Clean & Fast Code",
    "User-first Design",
  ];

  final Map<String, bool> status = {
    "Open for freelance" : true,
    "Building cool apps" : true,
    "Constantly learning": true,
  };
}

🛠️ Tech Stack
<div align="center">
📱 Flutter & Mobile
Show Image
Show Image
Show Image
Show Image
Show Image
Show Image
🎨 UI/UX Design
Show Image
Show Image
Show Image
Show Image
🧰 Tools & Dev
Show Image
Show Image
Show Image
Show Image
</div>

🎨 UI/UX Design Skills
AreaSkills📐 Mobile UIApp screens, Onboarding, Dashboard🎯 UX ResearchUser flows, Wireframes, Prototypes✨ AnimationsMicro-interactions, Lottie, Flutter animations🎨 VisualTypography, Color theory, Design systems🔧 ToolsFigma, Adobe XD, Protopie

📊 GitHub Analytics
<div align="center">
  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=ZafarKodirov&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true&hide_border=true&bg_color=05070f&title_color=00e5ff&icon_color=a78bfa&text_color=94a3b8"/>
  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=ZafarKodirov&layout=compact&theme=tokyonight&hide_border=true&bg_color=05070f&title_color=00e5ff&text_color=94a3b8"/>
</div>
<div align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=ZafarKodirov&theme=tokyonight&hide_border=true&background=05070f&stroke=00e5ff&ring=a78bfa&fire=f472b6&currStreakLabel=00e5ff" />
</div>
<br/>
<!-- Snake animation -->
<div align="center">
  <img src="https://raw.githubusercontent.com/ZafarKodirov/ZafarKodirov/output/github-contribution-grid-snake-dark.svg" alt="snake animation"/>
</div>

🔥 Featured Projects
<div align="center">
ProjectTypeDescription🚨 Scam Alert AppFlutter + UIFoydalanuvchilarni firibgarlikdan himoya qiluvchi ilova🔐 Secure GalleryFlutter + UIShifrlangan, minimal dizaynli galereya🛵 Delivery AppFlutterReal-time kuzatish va onboarding UI💰 Expense TrackerFlutter + UIDashboard dizaynli moliya boshqaruvi📅 Work SchedulerFlutter + UIClean calendar va drag & drop interfeys📍 Map TrackerFlutterCustom map UI bilan joylashuv kuzatish
</div>

📚 Currently Learning

🎨 Advanced UI Animations — Flutter Hero, Staggered, Rive
🏗️ Design Systems — Component libraries, Tokens
⚡ Advanced Flutter — Clean Architecture, BLoC
🌀 3D in Flutter — Spline, Three.js integration


🎯 Goals
✦ Flutter + UI/UX expert bo'lish
✦ Pixel-perfect, production-ready ilovalar yaratish  
✦ Freelance orqali global mijozlar bilan ishlash
✦ Open source proektlarga hissa qo'shish

📫 Connect With Me
<div align="center">
  <a href="https://t.me/YOUR_USERNAME">
    <img src="https://img.shields.io/badge/Telegram-2CA5E0?style=for-the-badge&logo=telegram&logoColor=white"/>
  </a>
  <a href="https://github.com/ZafarKodirov">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
  <a href="mailto:YOUR_EMAIL@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
</div>
<br/>

<div align="center">
  <img src="https://komarev.com/ghpvc/?username=ZafarKodirov&style=for-the-badge&color=00e5ff&label=PROFILE+VIEWS"/>
</div>
<!-- Footer -->
<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=120&section=footer" width="100%"/>
</div>
